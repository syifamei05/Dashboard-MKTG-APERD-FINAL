...
def home(request):
    ...
    # Build the month_totals array
    # monthly_all already has .values('m','total'), so:
    month_totals = [float(d['total'] or 0) for d in monthly_all]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                product.aperd.add(product_data.aperd)
